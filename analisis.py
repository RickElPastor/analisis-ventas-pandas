...
if df.empty:
    print("No se encontraron datos en el archivo ventas.csv")
else:
    # Métricas generales
    total = df["venta"].sum()
    ...
    with open("Reporte.txt", "w") as archivo:
        ...
        mas_gasto = resultado.index[0]
        gasto_mas = resultado.iloc[0]
    
        archivo.write(f"- {mas_gasto}: ${gasto_mas:,.2f}\n")

        archivo.write("\nCliente que menos gastó\n")
        menos = resultado.sort_values(ascending=True)
        menos_gasto = menos.index[0]
        gasto_menos = menos.iloc[0]

        archivo.write(f"- {menos_gasto}: ${gasto_menos:,.2f}\n")